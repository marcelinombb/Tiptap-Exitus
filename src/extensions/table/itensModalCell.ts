...
function saveBorderValue({ dropdown, editor }: DropDownEventProps) {
  ...
  const borderStyle = nodeAfter?.attrs?.style != undefined && nodeAfter?.attrs?.style?.border != undefined ? nodeAfter?.attrs.style.border : ''
  const [size = '', border = 'none', color = ''] = borderStyle.split(' ')
...
function saveBackValue({ dropdown, editor }: DropDownEventProps) {
  ...
  const backgroundStyle =
    nodeAfter?.attrs?.style != undefined && nodeAfter?.attrs?.style?.background != undefined ? nodeAfter?.attrs.style.background : ''
  const back = backgroundStyle.split(' ')
